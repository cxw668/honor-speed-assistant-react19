...
项目核心业务逻辑所在地，集成了基于英雄属性的动态博弈分析算法。

- **核心技术实现**：
  - **关系链实时推导**：利用 `useMemo` 对 10 名英雄进行交叉比对，实时提取“最佳搭档”、“压制”与“被压制”关系。
  - **交互状态机**：维护 `selectingSlot` 状态，实现点击跳转、自动环形寻空（Next Empty Slot Search）等智能化选人流。
  - **视口与滚动优化**：通过 `calc(100vh - offset)` 锁定主容器高度，并结合 `ResizeObserver` 动态监测分析结果区域，实现精细化的局部滚动提示与遮罩效果。

- **交互痛点与架构演进**：
  - **选人盲区优化**：初始设计中选择弹窗采用绝对定位覆盖层，导致用户在选择时无法直观感知已选阵容。**解决方案**：重构布局架构，将选择区域下移至槽位下方并采用流式布局，确保选人过程与阵容预览实时无缝衔接。
  - **渲染性能与闪烁修复**：
    - **问题定位**：由于 `TeamSlot` 定义在父组件内部，导致每次父组件更新状态时该组件类型被重新创建（Remounting），引发严重的图片闪烁。
    - **深度优化方案**：
      1. **组件解耦**：将 `TeamSlot` 移至外部定义，并利用 **React.memo** 实现属性级记忆化，彻底消除重挂载开销。
      2. **引用稳定性**：使用 `useCallback` 包装点击回调，防止因函数引用变动导致的无效重绘。
      3. **视觉降噪**：移除槽位容器的全局 `transition` 与图片动画，确保选中态切换为瞬时响应，视觉体验平滑。
